...
do
    ...
    if not player_find() then
        hook.Add( "InitPostEntity", "PlayerInit", function()
            if player_find() then return end
...
do
    ...
    local player_iteratorPVS = ash_player.iteratorPVS
...
do
    ...
    ---@type table<ash.player.HUD.Name, boolean>
    local defaults = {
    ...
    timer.Create( "ShouldDrawHUD", 0.25, 0, function()
        for i = 1, name_count, 1 do
            ...
            if pl ~= nil then
                ...
                local success, result = xpcall( hook_Run, ErrorNoHalt, "ash.player.ShouldDrawHUD", pl, name )
                if success and result ~= nil then
                    should_draw[ name ] = result == true
                    goto skip
                end

                should_draw[ name ] = defaults[ name ]
                ::skip::
            end
        end
    end )
